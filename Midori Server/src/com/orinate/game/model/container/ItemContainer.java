 ...
 * 
 */
public final class ItemContainer<T extends Item> implements Serializable {
	...
	public ItemContainer(int size, boolean alwaysStackable) {
		data = new Item[size];
			...
			if (item.getAmount() > 1) {
				if (freeSlots() >= item.getAmount()) {
					for (int i = 0; i < item.getAmount(); i++) {
						int index = freeSlot();
						data[index] = item.clone(item.getId(), 1);
					}
					return true;
				} else {
					return false;
			...
			if (data[i] != null) {
				if (data[i].getId() == item.getId()) {
					...
					if (amt > toRemove) {
						...
						amt -= toRemove;
						toRemove = 0;
						data[i] = data[i].clone(data[i].getId(), amt);
						return removed;
					} else {
						removed += amt;
						toRemove -= amt;
	...
	public ItemContainer<Item> asItemContainer() {
		ItemContainer<Item> c = new ItemContainer<Item>(data.length, this.alwaysStackable);
		...
		if (data[preferredSlot] != null) {
			if (data[preferredSlot].getId() == item.getId()) {
				...
				if (amt > toRemove) {
					...
					set2(preferredSlot, data[preferredSlot].clone(data[preferredSlot].getId(), amt));
					return removed;
				} else {
					removed += amt;
					toRemove -= amt;
			...
			if (data[i] != null) {
				if (data[i].getId() == item.getId()) {
					...
					if (amt > toRemove) {
						...
						// amt);
						set2(i, data[i].clone(data[i].getId(), amt));
						return removed;
					} else {
						removed += amt;
						toRemove -= amt;
	...
	public void addAll(ItemContainer<T> container) {
	...
	public boolean hasSpaceFor(ItemContainer<T> container) {
		for (int i = 0; i < container.getSize(); i++) {
			T item = container.get(i);
			if (item != null) {
