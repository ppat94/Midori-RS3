...
public class RegionMap {
	...
	public void addMask(int plane, int x, int y, int mask) {
		if (x >= 64 || y >= 64 || x < 0 || y < 0) {
			Location tile = new Location(regionX + x, regionY + y, plane);
			int regionId = tile.getRegionId();
			int newRegionX = (regionId >> 8) * 64;
			int newRegionY = (regionId & 0xff) * 64;
			if (clippedOnly) {
				World.getWorld().getRegion(tile.getRegionId()).getForceRegionMapClipped().addMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			} else {
				World.getWorld().getRegion(tile.getRegionId()).getForceRegionMap().addMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			}
			return;
		}
	...
	public void removeMask(int plane, int x, int y, int mask) {
		if (x >= 64 || y >= 64 || x < 0 || y < 0) {
			...
			int newRegionY = (regionId & 0xff) * 64;
			if (clippedOnly)
				World.getWorld().getRegion(tile.getRegionId()).getForceRegionMapClipped().removeMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			else
				World.getWorld().getRegion(tile.getRegionId()).getForceRegionMap().removeMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
			return;
		}
		masks[plane][x][y] = masks[plane][x][y] & (~mask);
	}
