...
/**
 * Handles the binding shot ability.
 * 
 * @author Emperor
 * 
 */
public final class BindingShot extends Ability {
	...
	/**
	 * Constructs a new {@code BindingShot} {@code Object}.
	 */
	public BindingShot() {
		super(0, 25, AbilityType.BASIC, 37, 14664, CombatStyle.RANGE, new SkillRequirement(Skills.RANGE, 15));
	}

	@Override
	public Ability newInstance() {
		return new BindingShot();
	}

	@Override
	public boolean register() {
		return AbilityRepository.register(1452, 2, this);
	}
	...
	public boolean meetsRequirements(Entity e, Entity target) {
		...
		if (weapon != null && weapon.getDefinitions().getWeaponType() == ItemDefinition.CROSSBOW_WEAPON_TYPE && ((Player) e).getEquipment().isDual()) {
            e.getAnimator().animate(Animation.create(18461, Priority.HIGH));
        } else if (weapon != null && weapon.getDefinitions().getWeaponType() == ItemDefinition.CROSSBOW_WEAPON_TYPE && ((Player) e).getEquipment().is2h()) {
            e.getAnimator().animate(Animation.create(18462, Priority.HIGH));
        } else if (weapon != null && weapon.getDefinitions().getWeaponType() == ItemDefinition.BOW_WEAPON_TYPE) {
            e.getAnimator().animate(Animation.create(18459, Priority.HIGH));
            //target.getAnimator().animate(null, new Graphics(3488));
		} else {
			e.getAnimator().animate(Animation.create(18461, Priority.HIGH));
		}
		return true;
