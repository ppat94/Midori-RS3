...
/**
 * Handles the fragmentation shot ability.
 * 
 * @author Emperor
 * 
 */
public final class FragmentationShot extends Ability {
	...
	/**
	 * Constructs a new {@code FragmentationShot} {@code Object}.
	 */
	public FragmentationShot() {
		super(0, 25, AbilityType.BASIC, 85, 14667, CombatStyle.RANGE, new SkillRequirement(Skills.RANGE, 25));
	}

	@Override
	public Ability newInstance() {
		return new FragmentationShot();
	}

	@Override
	public boolean register() {
		return AbilityRepository.register(1452, 5, this);
	}
	...
	public boolean fire(Entity user, Entity target) {
		...
		int max = (int) (getMaximumDamage(user, false, Skills.RANGE) * (1.0 + (RANDOM.nextDouble() * 1.88)));
			double acc = getHitAccuracy(user, target, 0, Skills.RANGE);
			if (acc > 0.1 && !AfterMath.stun(target)) {
				target.getImpactHandler().impact(max, user, HitType.RANGE_DAMAGE);
                bleed(user, target, max, 5, 3.0);
			}
		return true;
	}
