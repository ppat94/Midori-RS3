...
public final class Huffman {
	...
	public static int sendEncryptMessage(OutputStream stream, String message) {
		try {
			...
			byte[] messageData = StringUtil.formatText(message).getBytes();
	...
	public static final String readEncryptedMessage(InBuffer stream) {
		return readEncryptedMessage(32767, stream);
	}

	public static final String readEncryptedMessage(int maxLength, InBuffer stream) {
		try {
			int messageDataLength = stream.getUnsignedSmart();
			if (messageDataLength > maxLength)
				messageDataLength = maxLength;
			byte[] messageData = new byte[messageDataLength];
			stream.skipBytes(decryptMessage(messageData, messageDataLength, stream.buffer(), stream.offset(), 0));
			String message = StringUtil.getUnformatedMessage(messageDataLength, 0, messageData);
			return message;
		} catch (Throwable e) {
			return "";
		}
